...
import DashboardSidebarGovContainers from "./DashboardSidebarGovContainers"
  ...
  const handleClick = (e, titleProps) => {
    ...
    const addGovContainers = () => {
        if (userGovernments ===  undefined){
            ...
            return userGovernments.map((gov, index)=><DashboardSidebarGovContainers key={index} gov={gov} index={index} handleClick={handleClick} activeIndex={activeIndex} setMadeChange={setMadeChange} setActiveIndex={setActiveIndex}/>)
        }
    }
    ...
    return (
        <section className="dashboardPageContainer">
            ...
            <h3>Add Government to Dashboard</h3>
